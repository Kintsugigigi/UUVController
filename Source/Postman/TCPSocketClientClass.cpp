 ...
 * *********/
/* 客户端 建立连接 */
void TCPSocketClientClass::createTCPClient(std::string targetAddr, int targetPort)
{
    //创建客户端socket
    ...
    // 设置 ip
    const char * ipChar = targetAddr.data();
    //设置 端口号
    int nPort = targetPort;
    // socket 设置信息结构体
...
void TCPSocketClientClass::startGetMessage()
{
    // 服务端接收消息 线程
    getClientThread = thread(TCPSocketClientClass::getClientMessage_pth, this);
}

void TCPSocketClientClass::startSendMessage()
{
    // 服务端发送消息 线程
