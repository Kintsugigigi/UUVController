...
void ControlMasterClass::pathLoop(void * __this)
{
    ...
    //当前目标点
    int index = 0;
    int max_num = sizeof(_this->targetMenu.mission_path_ary) / sizeof(_this->targetMenu.mission_path_ary[0]);
    while (true)
    {
        //如果模式发生变化，则清空子状态
        ...
        {   //子状态清空
            ...
            index = 0;
            printf("模式{0}-切换成-模式{1}",_this->robotLastMode.mission_mode, _this->robotNowMode.mission_mode);
        }
